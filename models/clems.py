...
class CLEMS:
    ...
    def fit(self, x_train, y_train):
        ...
        bb = np.ascontiguousarray(y_train).view(
            np.dtype((np.void, y_train.dtype.itemsize * y_train.shape[1]))
        )
        ...
        for i in range(num_uq):
            for j in range(num_uq):
                delta[i, num_uq + j] = np.sqrt(
                    self.dis(self.y_train_uq[None, i], self.y_train_uq[None, j])
                )
                delta[num_uq + j, i] = delta[i, num_uq + j]

        # calculate MDS embedding
        mds = MDSW(
            n_components=self.z_dim,
            n_uq=num_uq,
            uq_weight=uq_weight,
            max_iter=300,
            eps=1e-6,
        ...
        # mds = MDS(n_components=self.z_dim, max_iter=300, eps=1e-6, dissimilarity="precomputed",n_init=8, n_jobs=1)
        ...
        # train regressor
        self.rgrs = [self.base_learner(**self.params) for i in range(self.z_dim)]
