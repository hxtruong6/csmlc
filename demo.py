...
alg_pcc_f1 = PCC("f1", RandomForestClassifier, params, y_test.shape[0])
...
# ham_br, rank_br, f1_br, acc_br = (
#     hamming_loss(y_test, p_br).mean(),
#     rank_loss(y_test, p_br).mean(),
#     f1_score(y_test, p_br).mean(),
#     accuracy_score(y_test, p_br).mean(),
# )
# ham_cc, rank_cc, f1_cc, acc_cc = (
#     hamming_loss(y_test, p_cc).mean(),
#     rank_loss(y_test, p_cc).mean(),
#     f1_score(y_test, p_cc).mean(),
#     accuracy_score(y_test, p_cc).mean(),
# )
# ham_pcc, rank_pcc, f1_pcc, acc_pcc = (
#     hamming_loss(y_test, p_pcc_ham).mean(),
#     rank_loss(y_test, p_pcc_rank).mean(),
#     f1_score(y_test, p_pcc_f1).mean(),
#     accuracy_score(y_test, p_pcc_f1).mean(),
# )
ham_cft, rank_cft, f1_cft, acc_cft = (
    hamming_loss(y_test, p_cft_ham).mean(),
    rank_loss(y_test, p_cft_rank).mean(),
    f1_score(y_test, p_cft_f1).mean(),
    accuracy_score(y_test, p_cft_acc).mean(),
)
ham_clems, rank_clems, f1_clems, acc_clems = (
    hamming_loss(y_test, p_clems_ham).mean(),
    rank_loss(y_test, p_clems_rank).mean(),
    f1_score(y_test, p_clems_f1).mean(),
    accuracy_score(y_test, p_clems_acc).mean(),
)

show_title = "algorithm  hamming_loss  rank_loss  f1_score  accuracy_score"
show_bar = "============================================================"
# show_br = "       BR        {:.4f}     {:.4f}    {:.4f}          {:.4f}".format(
#     ham_br, rank_br, f1_br, acc_br
# )
# show_cc = "       CC        {:.4f}     {:.4f}    {:.4f}          {:.4f}".format(
#     ham_cc, rank_cc, f1_cc, acc_cc
# )
# show_pcc = "      PCC        {:.4f}     {:.4f}    {:.4f}          {:.4f}".format(
#     ham_pcc, rank_pcc, f1_pcc, acc_pcc
# )
show_cft = "      CFT        {:.4f}     {:.4f}    {:.4f}          {:.4f}".format(
    ham_cft, rank_cft, f1_cft, acc_cft
)
show_clems = "    CLEMS        {:.4f}     {:.4f}    {:.4f}          {:.4f}".format(
    ham_clems, rank_clems, f1_clems, acc_clems
)
